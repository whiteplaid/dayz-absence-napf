...
if (count(_preTestCargoCounts select 0) == 0) exitWith { tfv_IS_TRADING = false; systemChat format [tfv_NO_BP_WEAPONS,typeOf _backpack]; PVDZE_Server_Simulation = [_backpack, true]; publicVariableServer "PVDZE_Server_Simulation";};
systemChat format [tfv_VEHICLE_CONFIRM,typeOf _backpack];
PVDZE_Server_Simulation = [_backpack, false];
...
if (_tradeCancel) exitWith { tfv_IS_TRADING = false; PVDZE_Server_Simulation = [_backpack, true]; publicVariableServer "PVDZE_Server_Simulation"; };
...
/*	Probably not needed since simulation = false;
	if (([_vStartPos,_sWepCounts,_backpack,_pStartPos] call tfv_fnc_checkTrade)) exitWith {
	    _badTrade = true;
		titleText [tfv_CANCELLED_TRADE,"PLAIN DOWN"];
	};
*/
...
if (_canceltrade) exitWith { 
	...
	PVDZE_Server_Simulation = [_backpack, true];
	publicVariableServer "PVDZE_Server_Simulation"; 
	[objNull, player, rSwitchMove,""] call RE;
	player playActionNow "stop";
};
if (_badTrade) exitWith { titleText [tfv_CANCELLED_TRADE,"PLAIN DOWN"]; tfv_IS_TRADING = false; PVDZE_Server_Simulation = [_backpack, true]; publicVariableServer "PVDZE_Server_Simulation"; };
